...
- **Комплексная оценка ответов**: Использует BERTScore и TF-IDF для многоуровневой оценки качества ответов.
- **Интеллектуальная генерация фидбека**: Адаптивная система генерации обратной связи на основе анализа содержания ответов.

## Система оценки

### Метрики оценки ответов
- **BERTScore (F1)**: Семантическое сходство с эталонным ответом
- **TF-IDF сходство**: Совпадение технических терминов
- **Покрытие ключевых терминов**: Процент покрытия важных концепций
- **Итоговая оценка**: Взвешенная комбинация всех метрик

### Метрики оценки фидбека
- **Уникальность фидбека**: Насколько фидбек отличается от ответа пользователя
- **Достоверность фидбека**: Соответствие фидбека эталонному ответу
- **Общее качество**: Комплексная оценка качества генерации

### Адаптивная генерация фидбека
Система автоматически определяет тип ответа и адаптирует стратегию генерации фидбека:

- **Релевантные ответы**: Детальное сравнение с эталонным ответом, конкретные рекомендации по улучшению
- **Краткие ответы**: Указание на необходимость более развернутого ответа
- **Ответы без технических терминов**: Честное указание на отсутствие технической информации
- **Нерелевантные ответы**: Рекомендации по изучению темы и переформулированию ответа
...
- Получите детальный фидбек после каждого ответа:
  
  **Оценка ответа:**
  - Семантическое сходство (BERTScore): оценка смыслового соответствия эталону
  - Сходство терминов (TF-IDF): совпадение технических терминов
  - Покрытие ключевых терминов: процент покрытия важных концепций
  - Итоговая оценка: комплексная оценка качества ответа
  
  **Оценка фидбека:**
  - Уникальность фидбека: насколько фидбек добавляет новую информацию
  - Достоверность фидбека: соответствие фидбека эталонному ответу
  - Общее качество: итоговая оценка качества генерации фидбека
  
  **Структура фидбека:**
  - Сильные стороны: что хорошо в вашем ответе
  - Что улучшить: конкретные недостатки и пробелы
  - Рекомендации: практические советы по улучшению ответа
